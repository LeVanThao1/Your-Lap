...
async function loadUser() {
    ...
    // console.log(dataCT, dataCT.data.user.fullname);
    if(!token || dataCT.status !== 200) {
        $('.header__navbar-list').append(`<li class="header__navbar-item header__navbar-item--bold header__navbar-item--separate">Đăng ký</li>
        <li class="header__navbar-item header__navbar-item--bold">Đăng nhập</li>`)

        $('.header__navbar-user-name').click(() => {
            redirect('loginandregiter.html')
        })
    }
    else if (dataCT.status === 200){
        $('.header__navbar-user').html(
